...
void TEvaluator::SetInstance( char filename[] )
{
  ...
  while( 1 ){
    if( fscanf( fp, "%s", word ) == EOF ) {
      printf("Reached EOF without finding NODE_COORD_SECTION\n");
      break;
    }

    printf("Read keyword: %s\n", word);
    ...
    if( strcmp( word, "EDGE_WEIGHT_TYPE" ) == 0 ){
      ...
      assert( strcmp( word, ":" ) == 0 );
      fscanf( fp, "%s", type ); 
      printf("Found EDGE_WEIGHT_TYPE: %s\n", type);
    } 

    if( strcmp( word, "NODE_COORD_SECTION" ) == 0 ) {
      printf("Found NODE_COORD_SECTION\n");
      break;
    }
  }
  
  if( strcmp( word, "NODE_COORD_SECTION" ) != 0 ){
    printf( "Error in reading the instance: NODE_COORD_SECTION not found\n" );
    exit(0);
  }
