...
public:
  ...
  void DoIt( TIndi& tKid, const TIndi& tPa1, const TIndi& tPa2, int numOfKids,  /* Main procedure of EAX */
	     int flagP, int flagC[ 10 ], int** fEdgeFreq, TFeatureExtractor* featureExtractor, 
       const pybind11::object& get_action_mask_func, const pybind11::object& store_experience_func );
  ...
  bool VerifyTourIntegrity(const TIndi& indi, const std::string& stage_name);  /* Verify tour integrity */
  ...
  bool IsValidLinkStructure(int** temp_link);                    /* Validate link structure */
